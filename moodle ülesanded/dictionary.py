...
def exchange_keys_and_values(exchange_dict: dict) -> dict:
    ...
    for key in exchange_dict:
        key_value = exchange_dict[key]
        exchanged_dict[key_value] = key
...
def count_symbol_appearances(stringy: str) -> dict:
    ...
    for letter in stringy:
        letter_count_in_string = stringy.count(letter)
        string_dict[letter] = letter_count_in_string
...
def organise_by_first_symbol(strings: list) -> dict:
    ...
    organised_dict = {}
    for word in strings:
        if word[0] in organised_dict:
            organised_dict[word[0]].append(word)
        else:
            organised_dict[word[0]] = [word]
