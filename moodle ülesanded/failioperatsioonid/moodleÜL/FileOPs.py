...
def write_lines_to_file(filename: str, lines: list) -> None:
    ...
    with open(filename, "w") as file:
        for x in range(0, len(lines)):
            if len(lines) < 2 or x == len(lines) - 1:
                file.writelines(f"{lines[x]}")
            else:
                file.writelines(f"{lines[x]}\n")
...
def merge_dates_and_towns_into_csv(dates_filename: str, towns_filename: str, csv_output_filename: str) -> None:
    ...
    for name, date in dates_data.items():
        town = towns_data.get(name, '-')
        output_data.append([name, town, date])
        processed_names.add(name)

    for name, town in towns_data.items():
...
if __name__ == '__main__':
    write_lines_to_file("new_file", ['hello', 'world', 'stop'])
