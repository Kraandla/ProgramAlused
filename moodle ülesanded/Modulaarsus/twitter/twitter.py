...
def sort_hashtags_by_popularity(tweets: list) -> list:
    ...
    for tweet in tweets:
        ...
        hashtag_pos = tweet_length - tweet.content[::-1].find('#') - 1
        hashtag = tweet.content[hashtag_pos:tweet_length]

        if hashtag in hashtags:
            calc = hashtags[hashtag] + tweet.retweets
            hashtags[hashtag] = calc
        else:
            hashtags[hashtag] = tweet.retweets
    hashtags = sorted(hashtags, reverse=True)
    return hashtags
